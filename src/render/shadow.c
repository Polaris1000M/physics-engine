...
unsigned int shadowInit(Shadow* s, Camera* c, vec3 lightDir)
{
    ...
    shaderInit(&s->shader, "../src/render/shaders/shadow-vs.glsl", "../src/render/shaders/shadow-fs.glsl");
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, s->SHADOW_WIDTH, s->SHADOW_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);

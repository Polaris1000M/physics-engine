...
// stores coordinates of vertex and indices of neighbors in IcoVertex array
...
// prints IcoVertex data
void icoVertexPrint(IcoVertex* vertex)
{
    printf("(%f, %f, %f)\n", vertex->coords[0], vertex->coords[1],
           vertex->coords[2]);
    printf("(");
...
// returns index of the target neighbor (stored in target) in the provided
// vertex returns -1 if the index is not found
...
// does nothing if the target already exists in the vertex or there are no more
// default values
...
int icoVertexMidpoint(IcoVertex* icoVertices, unsigned int vertexOne,
                      unsigned int vertexTwo, vec3 target)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        for (int j = 0; j < 6; j++)
        {
            if (icoVertices[vertexOne].neighbors[i] ==
                icoVertices[vertexTwo].neighbors[j])
            {
                vec3 diff;
...
struct IcoFace
{
    ...
    IcoFace* next[4];  // next 4 triangles created from a subdivision of the
                       // current triangle
};
...
// the vertices and indices to propagate to child faces during subdivision
// process
// 0 indicates one of the parent's vertices while 1 indicates one of the
// new midpoints vertices
// midpoints are all assumed to be in counter clockwise order
static const unsigned int divisionPattern[4][3][2] = {{{0, 0}, {1, 0}, {1, 1}},
                                                      {{1, 0}, {0, 1}, {1, 2}},
                                                      {{1, 1}, {1, 2}, {0, 2}},
                                                      {{1, 2}, {1, 1}, {1, 0}}};

// indicates which of the parent vertex indices the current midpoint lies
// between
static const unsigned int midpointPattern[3][2] = {{0, 1}, {0, 2}, {1, 2}};

// recursively divides an IcoFace into 3 smaller IcoFaces
void icoFaceSubdivide(IcoFace* face, IcoVertex* icoVertices, float defaultSize,
                      unsigned int* vertexIdx)
{
    ...
    if (face->next[0])
    {
        for (int i = 0; i < 4; i++)
        {
            icoFaceSubdivide(face->next[i], icoVertices, defaultSize,
                             vertexIdx);
        }
    ...
    for (int midpoint = 0; midpoint < 3; midpoint++)
    {
        ...
        vec3 target;
        glm_vec3_add(icoVertices[idxOne].coords, icoVertices[idxTwo].coords,
                     target);
        glm_vec3_scale_as(target, defaultSize, target);
        midpointIndices[midpoint] =
            icoVertexMidpoint(icoVertices, idxOne, idxTwo, target);
...
// recurs to the "leaf" child IcoFaces and generates new vertex data
void icoFacePopulate(IcoFace* face, IcoVertex* icoVertices, float* vertices,
                     unsigned int* faceIdx)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        glm_vec3_copy(icoVertices[face->indices[i]].coords,
                      vertices + *faceIdx);
        glm_normalize_to(icoVertices[face->indices[i]].coords,
                         vertices + *faceIdx + 3);
        (*faceIdx) += 6;
    }
}
...
unsigned int icoVertexCount()
{
    ...
    // the number of edges increases exponentially by a factor of 4 four each
    // subdivision
...
void sphereIcoMesh(float* mesh)
{
    ...
    const float radiusToSideRatio =
        1.0f / (sqrt(goldenRatio * goldenRatio + 1.0f) / 2.0f);
    ...
    icoVertices[1].coords[2] = sqrt(defaultSize * defaultSize -
                                    (sideLength * 0.5f) * (sideLength * 0.5f));
    mat4 rot = GLM_MAT4_IDENTITY;
    glm_rotate_y(rot, glm_rad(72.0f), rot);
    for (int i = 2; i <= 5; i++)
    {
        glm_mat4_mulv3(rot, icoVertices[i - 1].coords, 1.0f,
                       icoVertices[i].coords);
    }
    ...
    for (int i = 7; i <= 11; i++)
    {
        glm_mat4_mulv3(rot, icoVertices[i - 6].coords, 1.0f,
                       icoVertices[i].coords);
    ...
    for (int face = 0; face < 5; face++)
    {
        ...
        const unsigned int curTopIndices[3] = {0, face + topOffset,
                                               (face + 1) % 5 + topOffset};

        // lower pentagonal pyramid
        const unsigned int curBottomIndices[3] = {
            6, (face + 1) % 5 + bottomOffset, face + bottomOffset};

        // right side up middle faces
        const unsigned int curUpIndices[3] = {face + topOffset,
                                              (face + 4) % 5 + bottomOffset,
                                              face + bottomOffset};

        // upside down middle faces
        const unsigned int curDownIndices[3] = {
            face + bottomOffset, (face + 1) % 5 + topOffset, face + topOffset};
    ...
    // begin subdivisions
    unsigned int vertexIdx =
        12;  // the index the next faces will take during subdivision
    for (int i = 0; i < RECUR; i++)
    {
        for (int j = 0; j < 20; j++)
        {
            icoFaceSubdivide(icoFaces + j, icoVertices, defaultSize,
                             &vertexIdx);
        }
    }
    ...
    // the index the current face will have in the vertex buffer
    unsigned int faceIdx = 0;
    for (int i = 0; i < 20; i++)
    {
        icoFacePopulate(icoFaces + i, icoVertices, mesh, &faceIdx);
    }
        ...
        for (unsigned int sector = 0; sector < SECTORS; sector++)
        {
            ...
            for (unsigned int i = 0; i < 2; i++)
            {
                for (unsigned int j = 0; j < 2; j++)
                {
                    ...
                    x[idx] = defaultSize * cos(stackAngles[i]) *
                             sin(sectorAngles[j]);
                    y[idx] = defaultSize * sin(stackAngles[i]);
                    z[idx] = defaultSize * cos(stackAngles[i]) *
                             cos(sectorAngles[j]);
                }
            }
            ...
            if (stack == 0)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    glm_vec3_copy(cur, mesh + vertexIdx);
                    vertexIdx += 3;
